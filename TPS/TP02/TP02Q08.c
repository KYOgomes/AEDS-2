...
void selectionSort(Personagem personagens[], int i, int tam_vetor, int comp_mov[]) {
    ...
    for (int j = i + 1; j < tam_vetor; j++) {
        // Primeiro critério de ordenação: eyeColour
        if (strcmp(personagens[j].eyeColour, personagens[menor].eyeColour) < 0) {
            menor = j;
        }
        // Em caso de empate em eyeColour, desempate pelo nome
        else if (strcmp(personagens[j].eyeColour, personagens[menor].eyeColour) == 0 &&
