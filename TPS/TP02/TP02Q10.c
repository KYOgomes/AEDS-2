...
void selectionSort(Personagem personagens[], int i, int tam_vetor, int comp_mov[]) {
    ...
    for (int j = i + 1; j < tam_vetor; j++) {
        // Primeiro critério de ordenação: house
        if (strcmp(personagens[j].house, personagens[menor].house) < 0) {
            menor = j;
        }
        // Em caso de empate, desempate pelo nome
        else if (strcmp(personagens[j].house, personagens[menor].house) == 0 &&
