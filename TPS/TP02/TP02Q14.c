...
void selectionSort(Personagem personagens[], int i, int tam_vetor, int comp_mov[]) {
    ...
    for (int j = i + 1; j < tam_vetor; j++) {
        // Critério de ordenação: ID
        if (strcmp_lexicographic(personagens[j].id, personagens[menor].id) < 0) {
            menor = j;
        }
        comp_mov[0]++;
    }
...
// Função para comparar lexicograficamente duas strings
int strcmp_lexicographic(const char *s1, const char *s2) {
    while (*s1 && *s1 == *s2) {
        s1++;
        s2++;
    }
